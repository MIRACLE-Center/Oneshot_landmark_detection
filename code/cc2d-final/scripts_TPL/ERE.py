...
def two_d_softmax(x):
    exp_y = torch.exp(x)
    ret = exp_y / torch.sum(exp_y, dim=(2, 3), keepdim=True)
    if ret.min().isnan():
        print(torch.nonzero(ret.isnan()))
...
def nll_across_batch(output, target):
    ...
    temp = torch.sum(nll, dim=(2, 3))
    b, c = temp.shape
    mask = temp[1:] < temp[1:, :].max(0)[0]
    total_loss = temp[0].sum()
    total_loss += (mask * temp[1:]).sum()
...
class Tester(object):
    ...
    def test(self, net, mode="", dump_label=False):
        ...
        for img, _, __, ___, landmark_list, id_str, scale_rate in tqdm(self.dataloader_1):
            ...
            heatmap = two_d_softmax(heatmap.double())
...
if __name__ == "__main__":
    ...
    oneshot_dataset = dataset_class(cfg, 'Oneshot', size=cfg.train.input_size, id_shot=cfg.test.id_oneshot[0])
    ...
    dataset_class = select_dataset_ERE(args.dataset)
    dataset = dataset_class(cfg, mode='Train', ssl_dir=ssl_dir,\
                                  size=cfg.train.input_size, pseudo=True)

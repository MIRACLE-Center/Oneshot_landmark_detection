...
class Chest_Base(data.Dataset):
    ...
    def __init__(self, pathDataset, size=[384, 384], mode='Train', id_shot=None):
        ...
        self.pth_Image = os.path.join(pathDataset, 'pngs')
        self.pth_Label = os.path.join(pathDataset, 'labels')

        # file index
        files = [i[:-4] for i in sorted(os.listdir(self.pth_Image))][:279]
        n = len(files)
        ...
        self.images = {item['ID']: Image.open(os.path.join(self.pth_Image, item['ID']+'.png')).convert('RGB')\
    ...
    def get_landmark_gt(self, name, get_scale_rate=False):
        ...
        scale_rate = [1.0 * 512 / self.size[0], 1.0 * 512 / self.size[1]]
...
class Chest_SSL_Train(Chest_Base):
...
class Chest_SSL_Infer(Chest_Base):
...
class Chest_TPL_Heatmap(Chest_Base):
...
class Chest_TPL_Voting(Chest_Base):
...
class Chest_SSL_SAM(Chest_Base):
